...
// For setting and getting gene node position of leaves
...
// For speciation node 
void Node::initializePrePlaces(int size){
  this->prePlace = std::vector<std::vector<int> >(size);
  // (std::vector<int> *) malloc(size * sizeof(std::vector<int>))
							   ;
  // this->prePlace = (int *) malloc(size * sizeof(int));
    for(int i=0 ; i<size ; i++) 
      // this->prePlace[i] = -1;
      this->prePlace[i] = std::vector<int>(2,-1);	
} 

void Node::initializePlaces(int size){
    this->numberOfPlaces = size;
    this->place = (int *) malloc(size * sizeof(int));
    for(int i=0 ; i<size ; i++) 
      this->place[i] = -1;	
} 

bool Node::setPlace(int idx,int nodeid) {
   
    if(this->place[idx] == -1) {
       this->place[idx] = nodeid;
       return true;
    }
...
int Node::getPlace(int idx){
  return this->place[idx];
}

int Node::searchPlace(int nodeid) {
    int size = this->numberOfPlaces;
    for(int i=0 ; i<size ; i++) 
      if( this->place[i] == nodeid)
              return i;
