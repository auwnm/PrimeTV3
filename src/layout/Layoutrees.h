...
struct nodeplace
{
    ...
    nodeplace(int i, double p) : id(i), place(p) {}

    bool operator < (const nodeplace& np) const
    {
        return (place < np.place);
    }
};
...
private:
    ...
    // This should return the y-coordinate of u or the closest right-most child of u mapping at vertex x
